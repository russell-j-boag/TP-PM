...
save(samples, file = "samples/sTPPM_B.RData")
...
# Save samples
save(samples, samples1, file = "samples/sTPPM_B.RData")
...
#                       verbose = TRUE)
# # Save samples
# save(samples, samples1, samples2, file = "samples/sTPPM_B.RData")
...
# Chains
pdf("plots/chains_B.pdf", height = 6, width = 8)
par(mfrow = c(3, 3))
...
# Chains by subject
pdf("plots/chains_by_sub_B.pdf", height = 6, width = 8)
...
save(post_preds, file = "deriv/post_preds_B.RData")

# Load posterior predictives
load("deriv/post_preds_B.RData")

# Plot fits to CDFs
pdf("plots/cdfs_B.pdf", height = 6, width = 8)
plot.pp.dmc(post_preds, "cdf", layout = c(2, 2), model.legend = FALSE)
dev.off()

# Subject-level fits
pdf("plots/cdfs_by_sub_B.pdf", height = 6, width = 8)
...
save(post_pred_sims, file = "deriv/post_pred_sims_B.RData")
...
head(parms)
save(parms, file = "deriv/map_parms_B.RData")
...
# # Correlations
# pdf("plots/pairs_B.pdf", height = 6, width = 8)
