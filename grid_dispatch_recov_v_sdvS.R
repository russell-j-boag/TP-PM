...
# Save samples
save(recov_samples, recov_samples1, file = "samples/recov_sTPPM_v_sdvS.RData")
...
#                       report = 10)
# # Save samples
# save(recov_samples, recov_samples1, recov_samples2, file = "samples/recov_sTPPM_v_sdvS.RData")


# Sampling plots ----------------------------------------------------------

# Load samples
load("samples/recov_sTPPM_v_sdvS.RData")

# samples1 <- samples2

# Chains
pdf("plots/chains_recov_v_sdvS.pdf", height = 6, width = 8)
par(mfrow = c(3, 3))
...
# Chains by subject
pdf("plots/chains_by_sub_recov_v_sdvS.pdf", height = 6, width = 8)
...
save(post_preds, file = "deriv/post_preds_recov_v_sdvS.RData")

# Load posterior predictives
load("deriv/post_preds_recov_v_sdvS.RData")

# Plot fits to CDFs
pdf("plots/cdfs_recov_v_sdvS.pdf", height = 6, width = 8)
plot.pp.dmc(post_preds, "cdf", layout = c(2, 2), model.legend = FALSE)
dev.off()

# Subject-level fits
pdf("plots/cdfs_by_sub_recov_v_sdvS.pdf", height = 6, width = 8)
...
save(post_pred_sims, file = "deriv/post_pred_sims_recov_v_sdvS.RData")
...
# head(parms)
save(parms, file = "deriv/map_parms_recov_v_sdvS.RData")
...
# # Correlations
# pdf("plots/pairs_recov_v_sdvS.pdf", height = 6, width = 8)
