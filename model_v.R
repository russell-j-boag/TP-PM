...
p.prior <- prior.p.dmc(
  ...
  p1 = c(p.vector),                           
  p2 = c(0.2, 0.2, rep(1, 3), rep(1.5, 37)), 
  lower = c(0.1, 0, rep(0, 3), rep(NA, 37)),
  upper = c(1, 10, rep(Inf, 3), rep(Inf, 37))
)
length(p.prior)
length(p.vector)
...
# save(dm, file = "samples/dmTPPM_v.RData")
...
# Load
print(load("samples/sTPPM_v.RData"))
...
# 
# # Save
# save(hsamples, file = "dmc/samples/hsTPPM_full.RData")