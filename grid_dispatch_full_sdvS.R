...
# Save samples
save(samples, samples1, file = "samples/sTPPM_full_sdvS.RData")
...
#                       report = 10)
# # Save samples
# save(samples, samples1, samples2, file = "samples/sTPPM_full_sdvS.RData")
...
# Chains
pdf("plots/chains_full_sdvS.pdf", height = 6, width = 8)
par(mfrow = c(3, 3))
...
# Chains by subject
pdf("plots/chains_by_sub_full_sdvS.pdf", height = 6, width = 8)
...
save(post_preds, file = "deriv/post_preds_full_sdvS.RData")

# Load posterior predictives
load("deriv/post_preds_full_sdvS.RData")

# Plot fits to CDFs
pdf("plots/cdfs_full_sdvS.pdf", height = 6, width = 8)
plot.pp.dmc(post_preds, "cdf", layout = c(2, 2), model.legend = FALSE)
dev.off()

# Subject-level fits
pdf("plots/cdfs_by_sub_full_sdvS.pdf", height = 6, width = 8)
...
# head(parms)
save(parms, file = "deriv/map_parms_full_sdvS.RData")
...
# # Correlations
# pdf("plots/pairs_full_sdvS.pdf", height = 6, width = 8)
